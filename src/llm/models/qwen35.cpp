...
//
// Faithful 1:1 port from Swift reference. Qwen3.5 is very similar to Qwen3-Next
// (hybrid GDN+attention) but has separate in_proj_{qkv,z,b,a} projections
...
void from_json(const nlohmann::json& j, Qwen35Configuration& c) {
    ...
    c.linear_num_value_heads = j.value("linear_num_value_heads", 64);
    c.linear_num_key_heads = j.value("linear_num_key_heads", 16);
    c.linear_key_head_dim = j.value("linear_key_head_dim", 192);
    c.linear_value_head_dim = j.value("linear_value_head_dim", 128);
    c.linear_conv_kernel_dim = j.value("linear_conv_kernel_dim", 4);
    ...
    // Parse rope_scaling: check rope_parameters first, then rope_scaling
    if (j.contains("rope_parameters") && !j["rope_parameters"].is_null()) {
        ...
        // Normalize rope_type -> type
        if (scaling.find("type") == scaling.end()) {
            auto rt = scaling.find("rope_type");
            if (rt != scaling.end()) scaling["type"] = rt->second;
        }
...
Qwen35RMSNormGated::Qwen35RMSNormGated(int dimensions, float eps)
    : weight_(mx::ones({dimensions})), eps_(eps)
{}

mx::array Qwen35RMSNormGated::operator()(const mx::array& x,
...
std::unordered_map<std::string, mx::array*> Qwen35RMSNormGated::weight_map() {
    return {{"weight", &weight_}};
}

// --- Qwen35GatedDeltaNet ---
// Swift: Qwen35GatedDeltaNet -- linear attention
// Key difference from Qwen3-Next: separate projections for qkv, z, b, a

Qwen35GatedDeltaNet::Qwen35GatedDeltaNet(const Qwen35Configuration& args)
    : hidden_size_(args.hidden_size),
      ...
      // MLX conv1d weight shape: [C_out, 1, K] for depthwise (groups=C_out)
      conv1d_weight_(mx::zeros({conv_dim_, 1, args.linear_conv_kernel_dim})),
      // Separate projections (unlike Qwen3-Next which fuses qkvz and ba)
...
mx::array Qwen35GatedDeltaNet::operator()(
    ...
    MambaCache* cache)
{
    ...
    // Separate projections (Swift: inProjQKV, inProjZ, inProjB, inProjA)
    ...
    // Q/K norms with inv_scale baked in
    float inv_scale = std::pow(static_cast<float>(head_k_dim_), -0.5f);
...
std::unordered_map<std::string, mx::array*> Qwen35GatedDeltaNet::weight_map() {
...
// --- Qwen35Attention ---
// Swift: Qwen35Attention -- standard attention for full-attention layers
// q_proj outputs 2x head_dim (queries + sigmoid gate)

Qwen35Attention::Qwen35Attention(const Qwen35Configuration& args)
...
mx::array Qwen35Attention::operator()(const mx::array& x,
...
std::unordered_map<std::string, mx::array*> Qwen35Attention::weight_map() {
    std::unordered_map<std::string, mx::array*> map = {
...
Qwen35MLP::Qwen35MLP(int dimensions, int hidden_dimensions)
...
// Swift: Qwen35SparseMoeBlock -- Sparse MoE with shared expert

Qwen35SparseMoeBlock::Qwen35SparseMoeBlock(const Qwen35Configuration& args)
...
mx::array Qwen35SparseMoeBlock::operator()(const mx::array& x) {
...
std::unordered_map<std::string, mx::array*> Qwen35SparseMoeBlock::weight_map() {
    std::unordered_map<std::string, mx::array*> map;
...
// --- Qwen35DecoderLayer ---
// Swift: Qwen35DecoderLayer -- either linear attn or standard attn, with MLP or MoE

Qwen35DecoderLayer::Qwen35DecoderLayer(const Qwen35Configuration& args, int layer_idx)
      ...
      rms_norm_eps_(args.rms_norm_eps)
{
    ...
    // Swift: numExperts > 0 determines MoE (no decoder_sparse_step check in Qwen35)
...
mx::array Qwen35DecoderLayer::operator()(
    const mx::array& x,
...
std::unordered_map<std::string, mx::array*> Qwen35DecoderLayer::weight_map() {
    std::unordered_map<std::string, mx::array*> map;
    if (is_linear_) {
...
Qwen35ModelInner::Qwen35ModelInner(const Qwen35Configuration& args)
...
mx::array Qwen35ModelInner::operator()(const mx::array& inputs, std::vector<KVCache>* cache) {
    ...
    // SSM mask: always nullopt (Swift createSSMMask returns nil when cache has conv_state)
...
Qwen35Model::Qwen35Model(const Qwen35Configuration& args)
...
PrepareResult Qwen35Model::prepare_impl(const LMInput& input, std::vector<KVCache>& cache, int ws) {
...
std::vector<KVCache> Qwen35Model::new_cache_impl(const GenerateParameters& params) {
...
Qwen35Model::sanitize_impl(std::unordered_map<std::string, mx::array> weights) {
    ...
    // Remove lm_head for tied embeddings
...
void Qwen35Model::load_weights(const std::unordered_map<std::string, mx::array>& weights) {
